...
\[
PPP_{next_{ij}} = \beta_0 + \beta_1 \times PPP_{ij} + \beta_2 \times GP_{ij} + \beta_3 \times EFG\_PCT_{ij} + \beta_4 \times FTA\_FREQ_{ij} + \beta_5 \times TOV\_FREQ_{ij} + \beta_6 \times FREQ_{ij} + u_{0j} + \varepsilon_{ij}
\]
...
model <- lmer(PPP_next ~ PPP + GP + EFG_PCT + FTA_FREQ + TOV_FREQ + FREQ + (1 | TEAM_ABB), data = nba_data_clean |> filter(PLAY_TYPE == "Isolation"))
